...
function StudentLayoutContent({ children }: { children: React.ReactNode }) {
  ...
  const navigation = [
    ...
    { name: t("sidebar.myCourses"), href: "/student/dashboard/my-courses", icon: BookOpen },
    { name: t("sidebar.myQuizzes"), href: "/student/dashboard/my-quizzes", icon: ClipboardList },
    { name: t("sidebar.certificates"), href: "/student/dashboard/certificates", icon: Award },
    { name: t("sidebar.schedule"), href: "/student/dashboard/schedule", icon: Calendar },
    { name: t("sidebar.aiAssistant"), href: "/student/chatbot", icon: MessageSquare },
        ...
        <div className={`fixed inset-y-0 left-0 flex w-72 flex-col bg-gradient-to-b from-white via-white to-slate-50 shadow-2xl transition-transform duration-300 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
          <div className="flex h-20 items-center justify-between px-6 border-b border-slate-200 bg-white">
            <div className="flex items-center gap-3">
              ...
              <div>
              <span className="text-xl font-bold text-slate-900">{t("sidebar.ieltsAcademy")}</span>
              <p className="text-xs text-slate-500 font-medium">{t("sidebar.learningPlatform")}</p>
              </div>
            </div>
            <button 
      ...
      <div className="hidden lg:fixed lg:bottom-0 lg:top-24 lg:flex lg:w-72 lg:flex-col">
        <div className="flex flex-col flex-grow bg-gradient-to-b from-white via-white to-slate-50 border-r border-slate-200 shadow-lg">
          ...
          <div className="px-5 py-6 border-b border-slate-200 bg-gradient-to-br from-blue-50 via-white to-blue-50/30">
            <div className="bg-white rounded-2xl p-5 shadow-md border border-slate-100">
              <div className="flex items-center gap-4">
                ...
                <p className="text-base font-bold text-slate-900 truncate">{t("sidebar.studentPortal")}</p>
                <p className="text-sm text-slate-600 font-medium">{t("sidebar.welcomeBack")}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <nav className="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
            ...
            <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">{t("sidebar.menu")}</p>
            </div>
            {navigation.map((item) => {
              const isActive = item.href === "/student/dashboard" 
                ? pathname === item.href
