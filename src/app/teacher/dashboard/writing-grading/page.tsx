...
import { getPendingWritingSubmissions, getGradedWritingSubmissions, PendingWritingSubmission, GradedWritingSubmission } from "@/api/mockTest";
import { Button } from "@/components/ui/button";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { FileText, Clock, User, BookOpen, ArrowRight, CheckCircle, Eye } from "lucide-react";
import { format } from "date-fns";

const WritingGradingPage = () => {
  ...
  const { data: pendingData, isLoading: pendingLoading, error: pendingError } = useQuery({
    queryKey: ["pendingWritingSubmissions", pendingPage],
    queryFn: async () => {
      const result = await getPendingWritingSubmissions({ page: pendingPage, limit });
      console.log("[WritingGradingPage] Raw API response:", result);
      return result;
    },
  });

  const { data: gradedData, isLoading: gradedLoading, error: gradedError } = useQuery({
    queryKey: ["gradedWritingSubmissions", gradedPage],
    queryFn: async () => {
      const result = await getGradedWritingSubmissions({ page: gradedPage, limit });
      console.log("[WritingGradingPage] Graded API response:", result);
      return result;
    },
  });

  const handleGrade = (sectionResultId: string) => {
    router.push(`/teacher/dashboard/writing-grading/${sectionResultId}`);
  };
  ...
  const renderSubmissionCard = (submission: PendingWritingSubmission | GradedWritingSubmission, isGraded: boolean) => {
    const gradedSubmission = isGraded ? submission as GradedWritingSubmission : null;
        ...
        <CardContent>
          <div className="space-y-3">
            <div className="flex items-center gap-2 text-sm text-gray-600">
              ...
              <span>{submission.test_sections?.section_name || "Writing Section"}</span>
            </div>
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-gray-900 mb-2">Writing Grading</h1>
        <p className="text-gray-600">Review and grade student writing submissions</p>
      </div>
          ...
          {pendingSubmissions.length === 0 ? (
            <Card>
              <CardContent className="py-12 text-center">
                ...
                <p className="text-gray-600">All writing submissions have been graded.</p>
              </CardContent>
            </Card>
          ) : (
            <>
              <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {pendingSubmissions.map((submission) => renderSubmissionCard(submission, false))}
              </div>
          ...
          {gradedSubmissions.length === 0 ? (
            <Card>
              <CardContent className="py-12 text-center">
                ...
                <p className="text-gray-600">No writing submissions have been graded yet.</p>
              </CardContent>
            </Card>
          ) : (
            <>
              <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                {gradedSubmissions.map((submission) => renderSubmissionCard(submission, true))}
              </div>
...
export default WritingGradingPage;
