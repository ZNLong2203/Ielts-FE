...
import { deleteTeacherBlog, getTeacherBlogDetail } from "@/api/blog";
import toast from "react-hot-toast";
import ROUTES from "@/constants/route";

const TeacherBlogDetail = () => {
  ...
  const {
    ...
    queryKey: ["teacherBlog", slug],
    queryFn: () => getTeacherBlogDetail(slug),
    enabled: !!slug,
  });

  const { mutate: deleteBlog, isPending } = useMutation({
    mutationFn: deleteTeacherBlog,
    onError: (error: any) => {
      toast.error(error.response?.data?.message || "Failed to delete blog");
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["teacherBlogs"] });
      setOpen(false);
            ...
            <div className="flex items-center justify-between py-6">
              ...
              <div className="flex items-center space-x-3">
                <Button
                  variant="outline"
                  onClick={() =>
                    router.push(`${ROUTES.ADMIN_BLOGS}/${slug}/edit`)
                  }
...
export default TeacherBlogDetail;
